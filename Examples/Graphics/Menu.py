...
x = int(
    input(
        " Which Beam pattern do you want ? 1 : Rectangular , 2 : Circular or 3 : Diagonal ?"
    )
)
if x == 1:
...
if x == 2:
    ...
    theta = np.linspace(-np.pi / 2, np.pi / 2, 1000)
    fig = plt.figure(
        "Circular_Beam_pattern", figsize=(22, 12), facecolor="white", edgecolor="blue"
    )
    fig.set_size_inches(15, 10)
    ...
    P_potter1 = (
        ...
        - alpha
        * (((betaE1 / k) + np.cos(theta)) / (1 - (K11E1 / (k * np.sin(theta))) ** 2))
    ...
    P_potter2 = (
        ...
        - alpha
        * (((betaE2 / k) + np.cos(theta)) / (1 - (K11E2 / (k * np.sin(theta))) ** 2))
    ) * (special.jv(1, (k * r[0] * np.sin(theta))) / np.sin(theta))
    K11E3 = 3.832 / r[2]
    K11H3 = 1.841 / r[2]
    ...
    P_potter3 = (
        ...
        - alpha
        * (((betaE3 / k) + np.cos(theta)) / (1 - (K11E3 / (k * np.sin(theta))) ** 2))
    ) * (special.jv(1, (k * r[0] * np.sin(theta))) / np.sin(theta))
